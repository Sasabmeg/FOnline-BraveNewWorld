...
string __ValidNameLettersCommon;
string __ValidNameLettersCulture1;
string __ValidNameLettersCulture2;
...
class string
{
	...
	string@ get_opIndex(int) const;
	void set_opIndex(int, const string&in) const;
	uint length() const;
	uint rawLength() const;
	void rawResize(uint);
	uint8 rawGet(uint) const;
	void rawSet(uint, uint8);
...
class Critter
{
	...
	void SetAccess(uint8 access) const; // Arguments for this function have been guessed
	uint GetUID(uint8 index); // Arguments for this function have been guessed
	uint GetIp();
	string@ GetIpString();
	bool get_IsDisconnected() const;
	...
	bool TransitToMap(uint, uint16, uint16, uint8, bool = false);
	bool TransitToMap(uint, int, bool = false);
...
class Location
{
	...
	void Update();
	void EventFinish(bool);
	bool EventEnter(array<Critter@> &inout group, uint8 entrance); // Arguments for this function have been guessed
	bool IsEncounter();
	bool IsTown();
...
class Map
{
	...
	bool FindNearestGrid(uint16&inout, uint16&inout, uint16, uint);
	bool FindRandomGrid(uint16&inout, uint16&inout, uint16, uint);
	bool FindNearestGridApprox(uint16&inout, uint16&inout, bool);
	bool IsGrid(uint16, uint16);
	int16 GetGridDirection(uint16, uint16);
	void ProtoName(string&inout);
};
...
// Global functions
string@ substring(const string&in, int, uint);
int findFirst(const string&in, const string&in, int = 0);
int findLast(const string&in, const string&in, int = 0);
int findFirstOf(const string&in, const string&in, int = 0);
int findFirstNotOf(const string&in, const string&in, int = 0);
int findLastOf(const string&in, const string&in, int = 0);
int findLastNotOf(const string&in, const string&in, int = 0);
...
uint DecodeUTF8(const string&inout text, uint&inout length);
...
uint GetRegisteredPlayers(array<uint> &inout ids, array<string@> &inout names);
...
bool GetCurrentFunction(string&inout, string&inout, string&inout, bool = false);
