...
uint GetGenericFunctionBind(const ScriptString& scriptfunc, bool temp)
{
	...
	uint bind_id = FOClassic->ScriptBind(module.c_str(), func.c_str(), temp);
	if(!bind_id)
...
{
	if(!FOClassic->ScriptPrepare(bind_id))
	{
		Log("Failed to prepare context, bind id<%d>\n", bind_id);
		return -2;
	}

	FOClassic->ScriptSetArgObject(object);
	if(FOClassic->ScriptRunPrepared())
		return FOClassic->ScriptGetReturnedInt();
...
EXPORT void Global_RunScript(const ScriptString& scriptfunc, int p0, int p1, int p2, const ScriptString& p3)
{
	...
	uint bind_id = FOClassic->ScriptBind(module.c_str(), func.c_str(), true);
	if(!bind_id)
	{
		Log("Bind function fail: <%s> in module <%s>.\n", func.c_str(), module.c_str());
		return;
	...
	FOClassic->ScriptSetArgInt(p0);
	FOClassic->ScriptSetArgInt(p1);
	FOClassic->ScriptSetArgInt(p2);
	FOClassic->ScriptSetArgObject((void*)&p3);
