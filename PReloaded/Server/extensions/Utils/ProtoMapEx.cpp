...
MapObjectVec* FindNearestGroup(vector<MapObjectVec>& groups, uint16 hx, uint16 hy)
{
	...
	if(!closest_dist) return closest._Ptr;
...
MapObject* FindNearestObject(const MapObjectVec& vec, uint16 hx, uint16 hy, bool withPid = false, uint16 toMapPid = 0, bool withEntire = false, uint toEntire = 0)
{
...
MapObject* FindRandomObject(const MapObjectVec& vec, uint16 hx, uint16 hy, bool withPid = false, uint16 toMapPid = 0, bool withEntire = false, uint toEntire = 0)
{
	if(vec.empty()) return NULL;
	MapObjectVec objects;
	...
	return(objects.at(FOClassic->Random(0,objects.size()-1)));
}
...
EXPORT bool Map_FindNearestGridEntire(Map& imap, uint16& hx, uint16& hy, uint16 toMapPid, uint toEntire)
{
...
EXPORT bool Map_FindRandomGridEntire(Map& imap, uint16& hx, uint16& hy, uint16 toMapPid, uint toEntire)
{
...
EXPORT int16 Map_GetGridDirection(Map& map, uint16 hx, uint16 hy)
{
